...
stats = scipy.stats
os.chdir('F:/matthew/word2vec/google')
...
def make():
    ...
    os.system('gcc word2vec.c -o word2vec -lm -pthread -O3 -march=native -Wall -funroll-loops -Wno-unused-result')

def train_w2v():
    # set different parameters 
    os.system('./word2vec_mm -train data/enwik9.txt -save-vocab rslt/vocab.txt -output rslt/vector.txt -cbow 0 -size 200 -window 8 -negative 25 -hs 0 -sample 1e-4 -threads 20 -binary 0 -iter 1 -min-count 5000')


def load_rslt_from_c_output(file = 'word2vec/rslt/vector.txt'):
...
def similarity_test_one(U, vocab, data_name='men3000'):
    ...
    dataset = pd.read_csv('../word2vec/datasets/'+filename, header=None, delimiter=';').values
    ind1 = []
    ind2 = []
    vec2 = []
    model_dict=vocab
...
nearest_words(word='usa', vocab=vocab, inv_vocab=inv_vocab, U=word_embedding, top=20, display=False)
