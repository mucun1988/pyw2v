...
def nearest_words(word, vocab, inv_vocab, U, top=20):
    ...
    for i in range(1, top+1):
        nws.append({'word': inv_vocab[args[i]], 'score': round(cosines[args[i]],3)})
    return nws
...
def similarity_test_one(U, vocab, data_name='men3000'):
    ...
    dataset = pd.read_csv('./data/ws/'+filename, header=None, delimiter=';').values
    ind1 = []
    ind2 = []
    vec2 = []
    model_dict=vocab
...
if __name__ == "__main__":
    ...
    xx = nearest_words(word='china', vocab=vocab, inv_vocab=inv_vocab, U=word_embedding, top=20)
    ...
    find_b_star(tri_words=tri_words, U=word_embedding, vocab=vocab, inv_vocab=inv_vocab, normalized=False)
